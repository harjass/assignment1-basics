...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # not really a bias, more of a mask, but following the open ai / hf naming though
        self.register_buffer('bias', torch.tril(torch.ones(config.block_size, config.block_size))
                             .view(1, 1, config.block_size, config.block_size))
        
    def forward(self, x):
        B, T, C = x.size() # batch size, sequence length, embedding dimensionality (n_embd)
        # calculate query key values for all heads in a batch and move head forward to be the batch 
        # nh is number of heads, hs is headsize, C is number of heads * head size -> nh*hs
        # eg in gpt2 124M - n_heads = 12, hs = 64, so C = 768 channels in the transformer
        qkv = self.c_attn(x)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        config_args = {
            ...
            'gpt2-medium':  dict(n_layer=24, n_head=16, n_embd=1024), # 3550M params
        ...
        # copy while ensuring all params are aligned and match in names and shapes
...
while x.size(1) < max_length:
    ...
    with torch.no_grad():
        ...
        # do top-k sampling of the top 50
        topk_probs, topk_indices = torch.topk(probs, 50, dim=-1)
        # select a token from topk prbs
        ix = torch.multinomial(topk_probs, 1)
        # corrosponding indices
